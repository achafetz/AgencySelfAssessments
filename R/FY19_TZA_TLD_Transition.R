...
df_tld_del <- read_excel("data/TLD_transition_Deloitte.xlsx")
...
df_tld %>% 
  ...
  labs(x = NULL, y = NULL, 
       title = "EGPAF TLD patients as a share of TX_CURR",
...
df_tld_del %>% 
  ...
  ggplot(aes(x = phase, fill = phase, color = phase)) +
  geom_col(aes(y = TX_CURR), fill = "white", size = 1) + 
  geom_col(aes(y = TLD)) +
  geom_blank(aes(y = 1.1 * TX_CURR)) +
  geom_text(aes(y = TX_CURR, label = percent(TLD_share, 1)),
  ...
  scale_color_manual(values = c("#26456a", "#335B8E", "#739bcc")) +
  labs(x = NULL, y = NULL, 
       title = "Deloitte TLD patients as a share of TX_CURR",
       caption = "Deloitte Program Data") +
  theme_minimal() +
  theme(text = element_text(family = "Gill Sans MT"),
        plot.caption = element_text(color = "gray30", size = 10),
        legend.position = "none")

ggsave("out/TZA_SelfAssessment_Deloitte_TLD.png",
       dpi = 300, width = 4.1, height = 4.6)
